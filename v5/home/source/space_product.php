...
if($id) {
	...
	if($product['tag'] && empty($product['related'])) {
		...
		updatetable('productfield', array('related'=>addslashes(serialize(sstripslashes($product['related']))), 'relatedtime'=>$_SGLOBAL['timestamp']), array('productid'=>$product['productid']));//更新
	} else {
