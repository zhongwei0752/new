<?php if(!defined('IN_UCHOME')) exit('Access Denied');?><?php subtplcheck('template/default/space_cases_view|template/default/header|template/default/space_menu|template/default/space_click|template/default/space_comment_li|template/default/footer', '1370768404', 'template/default/space_cases_view');?><?php $_TPL['titles'] = array($cases['subject'], '成功案例'); ?>
...
<?php if($space['self']) { ?>
<h2 class="title"><img src="image/app/blog.gif" /><?=$_SN[$space['uid']]?>>成功案例</h2>

<?php } else { ?>
<?php $_TPL['spacetitle'] = "成功案例";
	$_TPL['spacemenus'][] = "<a href=\"space.php?uid=$space[uid]&do=$do&view=me\">TA的所有企业介绍</a>";
	$_TPL['spacemenus'][] = "<a href=\"space.php?uid=$space[uid]&do=cases&id=$cases[casesid]\">查看企业介绍</a>"; ?>
...
<div class="title">
<h1<?php if($cases['magiccolor']) { ?> class="magiccolor<?=$cases['magiccolor']?>"<?php } ?>><?=$cases['subject']?></h1>
<a href="cp.php?ac=cases" class="r_option" style="padding: 0 5px; height: 24px; border: none; background: #FF8901; color: #FFF; line-height: 20px; letter-spacing: 1px; cursor: pointer; ">发布</a>
<?php if($cases['friend']) { ?>
<span class="r_option locked gray"><?=$friendsname[$cases['friend']]?></span>
<?php } ?>
<?php if($cases['hot']) { ?><span class="hot"><em>热</em><?=$cases['hot']?></span><?php } ?>
<?php if($cases['friend']) { ?>
<span class="r_option locked gray">
<a href="space.php?uid=<?=$space['uid']?>&do=<?=$do?>&view=me&friend=<?=$cases['friend']?>" class="gray"><?=$friendsname[$value['friend']]?></a>
</span>
<?php } ?>
<?php if($cases['viewnum']) { ?><span class="gray">已有 <?=$cases['viewnum']?> 次阅读</span>&nbsp;<?php } ?>
 <span class="gray"><?php echo sgmdate('Y-m-d H:i',$cases[dateline],1); ?></span><br/><?php if($cases['company']) { ?><span class="gray">案例甲方:   <?=$cases['company']?></span><?php } ?>
<?php if($cases['tag']) { ?>
&nbsp; <a href="space.php?uid=<?=$cases['uid']?>&do=tag">标签</a>:&nbsp;
<?php if(is_array($cases['tag'])) { foreach($cases['tag'] as $tagid => $tagname) { ?>
<a href="space.php?uid=<?=$cases['uid']?>&do=tag&id=<?=$tagid?>"><?=$tagname?></a>&nbsp;
<?php } } ?>
<?php } ?>
</div>


<div id="cases_article" class="article <?php if($cases['magicpaper']) { ?> magicpaper<?=$cases['magicpaper']?><?php } ?>">
<div class="resizeimg">
...
<?php } ?>
<br/>
<?=$cases['message']?><br/><br/><br/>
<?php if($cases['messagecomment']) { ?>
<h2>案例甲方评论</h2><br/>
<?=$cases['messagecomment']?>
<?php } ?>
</div>
</div>
</div>
...
<a href="cp.php?ac=cases&casesid=<?=$cases['casesid']?>&op=edit">编辑</a><span class="pipe">|</span>
<a href="cp.php?ac=cases&casesid=<?=$cases['casesid']?>&op=delete" id="blog_delete_<?=$cases['casesid']?>" onclick="ajaxmenu(event, this.id)">删除</a>
<?php } ?>
...
<?php } ?>
评论 (<span id="comment_replynum"><?=$cases['replynum']?></span> 个评论)</h2>
