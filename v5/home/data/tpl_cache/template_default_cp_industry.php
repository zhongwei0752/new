<?php if(!defined('IN_UCHOME')) exit('Access Denied');?><?php subtplcheck('template/default/cp_industry|template/default/header|template/default/cp_topic_menu|template/default/footer|template/default/space_topic_inc', '1370765836', 'template/default/cp_industry');?><?php if(empty($_SGLOBAL['inajax'])) { ?>
...
<div class="tabs_header">
<ul class="tabs">
<?php if($industry['industryid']) { ?>
<li class="active"><a href="cp.php?ac=industry&industryid=<?=$industry['industryid']?>"><span>编辑</span></a></li>
<?php } ?>
<li<?php if(empty($industry['industryid'])) { ?> class="active"<?php } ?>><a href="cp.php?ac=industry"><span>发布</span></a></li>
<li><a href="space.php?uid=<?=$space['uid']?>&do=industry&view=me"><span>行业动态</span></a></li>
</ul>
</div>
<?php } ?>
...
标题：<input type="text" class="t_input" id="subject" name="subject" value="<?=$industry['subject']?>" size="60" onblur="relatekw();" placeholder="标题长度不超过80字符" />
<br/>
摘要：<input type="text" class="t_input" size="82" id="tag" name="tag" value="<?=$industry['tag']?>" placeholder="摘要可自己填写，也会自动获取！"> <input type="button" name="clickbutton[]" value="自动获取" class="button" onclick="relatekw();"  ><br/>
封面图片:<input type="file" name="files" value="<?=$industry['image1url']?>"/>
</td>
</tr>
<tr>
<td>
...
<th width="100">&nbsp;</th>
<td>
<input type="button" id="issuance" onclick="document.getElementById('industrybutton').click();" value="保存发布" class="submit" /></td>
</tr>
</table>
</div>
