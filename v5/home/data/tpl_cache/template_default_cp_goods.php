<?php if(!defined('IN_UCHOME')) exit('Access Denied');?><?php subtplcheck('template/default/cp_goods|template/default/header|template/default/cp_topic_menu|template/default/footer|template/default/space_topic_inc', '1375947455', 'template/default/cp_goods');?><?php if(empty($_SGLOBAL['inajax'])) { ?>
...
<a href="javascript:hideMenu();" class="float_del" title="关闭">关闭</a>
<div class="popupmenu_inner">
<form method="post" action="cp.php?ac=goods&op=edithot&goodsid=<?=$goodsid?>">
<p class="btn_line">
新的热度：<input type="text" name="hot" value="<?=$goods['hot']?>" size="5"> 
<input type="hidden" name="refer" value="<?=$_SGLOBAL['refer']?>" />
...
<div class="post_list container_12">
                         <span class="select_title grid_1">详细描述&nbsp;&nbsp;:</span>
                          <a id="doodleBox" href="magic.php?mid=doodle&showid=goods_doodle&target=uchome-ttHtmlEditor&from=editor" style="display:none"></a>
...
<input type="button" id="goodsbutton" name="goodsbutton" value="提交发布" onclick="validate(this);" style="display: none;" />
...
<div class="confirm_btn container_12">
                           ...
                           <input type="button" class="btn grid_2" id="issuance" onclick="document.getElementById('goodsbutton').click();" value="保存发布" class="submit" />
