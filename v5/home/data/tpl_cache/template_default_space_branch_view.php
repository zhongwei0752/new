<?php if(!defined('IN_UCHOME')) exit('Access Denied');?><?php subtplcheck('template/default/space_branch_view|template/default/header|template/default/space_menu|template/default/space_click|template/default/space_comment_li|template/default/footer', '1370766871', 'template/default/space_branch_view');?><?php $_TPL['titles'] = array($branch['subject'], '分支机构'); ?>
...
<?php if($space['self']) { ?>
<h2 class="title"><img src="image/app/blog.gif" /><?=$_SN[$space['uid']]?>>分支机构</h2>

<?php } else { ?>
<?php $_TPL['spacetitle'] = "分支机构";
	$_TPL['spacemenus'][] = "<a href=\"space.php?uid=$space[uid]&do=$do&view=me\">TA的所有企业介绍</a>";
	$_TPL['spacemenus'][] = "<a href=\"space.php?uid=$space[uid]&do=branch&id=$branch[branchid]\">查看企业介绍</a>"; ?>
...
<div class="title">
<h1<?php if($branch['magiccolor']) { ?> class="magiccolor<?=$branch['magiccolor']?>"<?php } ?>><?=$branch['subject']?></h1>
<a href="cp.php?ac=branch" class="r_option" style="padding: 0 5px; height: 24px; border: none; background: #FF8901; color: #FFF; line-height: 20px; letter-spacing: 1px; cursor: pointer; ">发布</a>
<?php if($branch['friend']) { ?>
<span class="r_option locked gray"><?=$friendsname[$branch['friend']]?></span>
<?php } ?>
<?php if($branch['hot']) { ?><span class="hot"><em>热</em><?=$branch['hot']?></span><?php } ?>
<?php if($branch['friend']) { ?>
<span class="r_option locked gray">
<a href="space.php?uid=<?=$space['uid']?>&do=<?=$do?>&view=me&friend=<?=$branch['friend']?>" class="gray"><?=$friendsname[$value['friend']]?></a>
</span>
<?php } ?>
<?php if($branch['viewnum']) { ?><span class="gray">已有 <?=$branch['viewnum']?> 次阅读</span>&nbsp;<?php } ?>
 <span class="gray"><?php echo sgmdate('Y-m-d H:i',$branch[dateline],1); ?></span><br/>
 <?php if($branch['place']) { ?>地址:<?=$branch['place']?>;<?php } else { ?>地址:未填;<?php } ?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
 <?php if($branch['telephone']) { ?>电话:<?=$branch['telephone']?>;<?php } else { ?>电话:未填;<?php } ?><br/>
 <?php if($branch['email']) { ?>邮箱:<?=$branch['email']?>;<?php } else { ?>邮箱:未填;<?php } ?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
 <?php if($branch['qq']) { ?>QQ:<?=$branch['qq']?>;<?php } else { ?>QQ:未填;<?php } ?><br/>
<?php if($branch['tag']) { ?>
&nbsp; <a href="space.php?uid=<?=$branch['uid']?>&do=tag">标签</a>:&nbsp;
<?php if(is_array($branch['tag'])) { foreach($branch['tag'] as $tagid => $tagname) { ?>
<a href="space.php?uid=<?=$branch['uid']?>&do=tag&id=<?=$tagid?>"><?=$tagname?></a>&nbsp;
<?php } } ?>
<?php } ?>
</div>


<div id="branch_article" class="article <?php if($branch['magicpaper']) { ?> magicpaper<?=$branch['magicpaper']?><?php } ?>">
<div class="resizeimg">
...
<a href="cp.php?ac=branch&branchid=<?=$branch['branchid']?>&op=edit">编辑</a><span class="pipe">|</span>
<a href="cp.php?ac=branch&branchid=<?=$branch['branchid']?>&op=delete" id="blog_delete_<?=$branch['branchid']?>" onclick="ajaxmenu(event, this.id)">删除</a>
<?php } ?>
...
<?php } ?>
评论 (<span id="comment_replynum"><?=$branch['replynum']?></span> 个评论)</h2>
