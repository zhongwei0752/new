<?php if(!defined('IN_UCHOME')) exit('Access Denied');?><?php subtplcheck('template/default/cp_product|template/default/header|template/default/cp_topic_menu|template/default/footer|template/default/space_topic_inc', '1370750469', 'template/default/cp_product');?><?php if(empty($_SGLOBAL['inajax'])) { ?>
...
<li class="active"><a href="cp.php?ac=product&productid=<?=$product['productid']?>"><span>编辑</span></a></li>
<?php } ?>
<li<?php if(empty($product['productid'])) { ?> class="active"<?php } ?>><a href="cp.php?ac=product"><span>发布</span></a></li>
<li><a href="space.php?uid=<?=$space['uid']?>&do=product&view=me"><span>产品介绍</span></a></li>
</ul>
</div>
<?php } ?>
...
标题：<input type="text" class="t_input" id="subject" name="subject" value="<?=$product['subject']?>" size="60" onblur="relatekw();" placeholder="标题长度不超过80字符" />
<br/>
价格：<input type="text" class="t_input" size="82" id="money" name="money" value="<?=$product['money']?>"><br/>
封面图片:<input type="file" name="files" value="<?=$product['image1url']?>"/>
...
<th width="100">&nbsp;</th>
<td>
<input type="button" id="issuance" onclick="document.getElementById('productbutton').click();" value="保存发布" class="submit" /></td>
</tr>
</table>
</div>
