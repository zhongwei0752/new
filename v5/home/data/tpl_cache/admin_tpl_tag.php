<?php if(!defined('IN_UCHOME')) exit('Access Denied');?><?php subtplcheck('admin/tpl/tag|admin/tpl/header|admin/tpl/side|admin/tpl/footer|template/default/header|template/default/footer', '1370600836', 'admin/tpl/tag');?><?php $_TPL['menunames'] = array(
		'index' => '管理首页',
		'config' => '站点设置',
		'privacy' => '隐私设置',
		'usergroup' => '用户组',
...
<tr>
<td><input type="<?php if($allowbatch) { ?>checkbox<?php } else { ?>radio<?php } ?>" name="ids[]" value="<?=$value['tagid']?>">
<a href="space.php?uid=<?=$space['uid']?>&do=tag&id=<?=$value['tagid']?>" target="_blank"><?=$value['tagname']?></a><?php if($value['close']) { ?>(<a href="admincp.php?ac=tag&close=<?=$value['close']?>">锁定</a>)<?php } ?></td>
<td><?=$value['blognum']?></td>
