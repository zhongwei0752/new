<?php if(!defined('IN_UCHOME')) exit('Access Denied');?><?php subtplcheck('template/default/space_industry_view|template/default/header|template/default/space_menu|template/default/space_click|template/default/space_comment_li|template/default/footer', '1370765831', 'template/default/space_industry_view');?><?php $_TPL['titles'] = array($industry['subject'], '行业动态'); ?>
...
<?php if($space['self']) { ?>
<h2 class="title"><img src="image/app/blog.gif" /><?=$_SN[$space['uid']]?>>行业动态</h2>

<?php } else { ?>
<?php $_TPL['spacetitle'] = "行业动态";
	$_TPL['spacemenus'][] = "<a href=\"space.php?uid=$space[uid]&do=$do&view=me\">TA的所有企业介绍</a>";
	$_TPL['spacemenus'][] = "<a href=\"space.php?uid=$space[uid]&do=industry&id=$industry[industryid]\">查看企业介绍</a>"; ?>
...
<div class="title">
<h1<?php if($industry['magiccolor']) { ?> class="magiccolor<?=$industry['magiccolor']?>"<?php } ?>><?=$industry['subject']?></h1>
<a href="cp.php?ac=industry" class="r_option" style="padding: 0 5px; height: 24px; border: none; background: #FF8901; color: #FFF; line-height: 20px; letter-spacing: 1px; cursor: pointer; ">发布</a>
<?php if($industry['friend']) { ?>
<span class="r_option locked gray"><?=$friendsname[$industry['friend']]?></span>
<?php } ?>
<?php if($industry['hot']) { ?><span class="hot"><em>热</em><?=$industry['hot']?></span><?php } ?>
<?php if($industry['friend']) { ?>
<span class="r_option locked gray">
<a href="space.php?uid=<?=$space['uid']?>&do=<?=$do?>&view=me&friend=<?=$industry['friend']?>" class="gray"><?=$friendsname[$value['friend']]?></a>
</span>
<?php } ?>
<?php if($industry['viewnum']) { ?><span class="gray">已有 <?=$industry['viewnum']?> 次阅读</span><?php } ?>
&nbsp; <span class="gray"><?php echo sgmdate('Y-m-d H:i',$industry[dateline],1); ?></span>
<?php if($industry['tag']) { ?>
&nbsp; <a href="space.php?uid=<?=$industry['uid']?>&do=tag">标签</a>:&nbsp;
<?php if(is_array($industry['tag'])) { foreach($industry['tag'] as $tagid => $tagname) { ?>
<a href="space.php?uid=<?=$industry['uid']?>&do=tag&id=<?=$tagid?>"><?=$tagname?></a>&nbsp;
<?php } } ?>
<?php } ?>
</div>


<div id="industry_article" class="article <?php if($industry['magicpaper']) { ?> magicpaper<?=$industry['magicpaper']?><?php } ?>">
<div class="resizeimg">
...
<a href="cp.php?ac=industry&industryid=<?=$industry['industryid']?>&op=edit">编辑</a><span class="pipe">|</span>
<a href="cp.php?ac=industry&industryid=<?=$industry['industryid']?>&op=delete" id="blog_delete_<?=$industry['industryid']?>" onclick="ajaxmenu(event, this.id)">删除</a>
<?php } ?>
...
<?php } ?>
评论 (<span id="comment_replynum"><?=$industry['replynum']?></span> 个评论)</h2>
