<?php if(!defined('IN_UCHOME')) exit('Access Denied');?><?php subtplcheck('template/default/cp_introduce|template/default/header|template/default/cp_topic_menu|template/default/footer|template/default/space_topic_inc', '1370751792', 'template/default/cp_introduce');?><?php if(empty($_SGLOBAL['inajax'])) { ?>
...
<div class="tabs_header">
<ul class="tabs">
<?php if($introduce['introduceid']) { ?>
<li class="active"><a href="cp.php?ac=introduce&introduceid=<?=$introduce['introduceid']?>"><span>编辑</span></a></li>
<?php } ?>
<li<?php if(empty($introduce['introduceid'])) { ?> class="active"<?php } ?>><a href="cp.php?ac=introduce"><span>发布</span></a></li>
<li><a href="space.php?uid=<?=$space['uid']?>&do=introduce&view=me"><span>企业介绍</span></a></li>
</ul>
</div>
<?php } ?>
...
标题：<input type="text" class="t_input" id="subject" name="subject" value="<?=$introduce['subject']?>" size="60" onblur="relatekw();" placeholder="标题长度不超过80字符" />
<br/>
摘要：<input type="text" class="t_input" size="82" id="tag" name="tag" value="<?=$introduce['tag']?>" placeholder="摘要可自己填写，也会自动获取！"> <input type="button" name="clickbutton[]" value="自动获取" class="button" onclick="relatekw();"  ><br/>
封面图片:<input type="file" name="files" value="<?=$introduce['image1url']?>" />
</td>
...
<th width="100">&nbsp;</th>
<td>
<input type="button" id="issuance" onclick="document.getElementById('introducebutton').click();" value="保存发布" class="submit" /></td>
</tr>
</table>
</div>
