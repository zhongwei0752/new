<?php if(!defined('IN_UCHOME')) exit('Access Denied');?><?php subtplcheck('template/default/cp_development|template/default/header|template/default/cp_topic_menu|template/default/footer|template/default/space_topic_inc', '1370764772', 'template/default/cp_development');?><?php if(empty($_SGLOBAL['inajax'])) { ?>
...
<?php } else { ?>
<h2 class="title"><img src="image/app/blog.gif" /><?=$_SN[$space['uid']]?>>产品动态</h2>
<div class="tabs_header">
<ul class="tabs">
<?php if($development['developmentid']) { ?>
<li class="active"><a href="cp.php?ac=development&developmentid=<?=$development['developmentid']?>"><span>编辑</span></a></li>
<?php } ?>
<li<?php if(empty($development['developmentid'])) { ?> class="active"<?php } ?>><a href="cp.php?ac=development"><span>发布</span></a></li>
<li><a href="space.php?uid=<?=$space['uid']?>&do=development&view=me"><span>产品动态</span></a></li>
</ul>
</div>
<?php } ?>
...
<tr>
<td>
分类：<select name="classid" id="classid" onchange="addSort(this)">
...
标题：<input type="text" class="t_input" id="subject" name="subject" value="<?=$development['subject']?>" size="60" onblur="relatekw();" placeholder="标题长度不超过80字符" />
<br/>
摘要：<input type="text" class="t_input" size="82" id="tag" name="tag" value="<?=$development['tag']?>" placeholder="摘要可自己填写，也会自动获取！"> <input type="button" name="clickbutton[]" value="自动获取" class="button" onclick="relatekw();"  ><br/>
封面图片:<input type="file" name="files" value="<?=$development['image1url']?>" />
</td>
</tr>
<tr>
<td>
...
<th width="100">&nbsp;</th>
<td>
<input type="button" id="issuance" onclick="document.getElementById('developmentbutton').click();" value="保存发布" class="submit" /></td>
</tr>
</table>
</div>
