<?php if(!defined('IN_UCHOME')) exit('Access Denied');?><?php subtplcheck('template/default/cp_job|template/default/header|template/default/cp_topic_menu|template/default/footer|template/default/space_topic_inc', '1370323034', 'template/default/cp_job');?><?php if(empty($_SGLOBAL['inajax'])) { ?>
...
<ul class="tabs">
<?php if($job['jobid']) { ?>
<li class="active"><a href="cp.php?ac=job&jobid=<?=$job['jobid']?>"><span>编辑</span></a></li>
<?php } ?>
<li<?php if(empty($job['jobid'])) { ?> class="active"<?php } ?>><a href="cp.php?ac=job"><span>发布</span></a></li>
<li><a href="space.php?uid=<?=$space['uid']?>&do=job&view=me"><span>人才招聘</span></a></li>
</ul>
</div>
<?php } ?>
...
<tr>
<td>
职位名称:<input type="text" class="t_input" id="subject" name="subject" value="<?=$job['subject']?>" size="77" onblur="relatekw();"  />
<br/>
封面图片:<input type="file" name="files" value="<?=$job['image1url']?>"/>
</td>
</tr>
<tr>
<td>职位描述：
...
<textarea name="messagecomment" style="width:750px;height:200px;"><?=$job['messagecomment']?></textarea>
</td>
</tr>

<tr><td>
电话:<input type="text" class="t_input" size="82" id="telephone" name="telephone" value="<?=$job['telephone']?>" > <br/>
邮箱:<input type="text" class="t_input" size="82" id="email" name="email" value="<?=$job['email']?>" > <br/>
待遇:<input type="text" class="t_input" size="82" id="money" name="money" value="<?=$job['money']?>" > <br/>

</td></tr>
<tr>
<td>其他：<br/>

<textarea name="othermessage" style="width:750px;height:200px;"><?=$job['othermessage']?></textarea>
</td>
</tr>
...
<th width="100">&nbsp;</th>
<td>
<input type="button" id="issuance" onclick="document.getElementById('jobbutton').click();" value="保存发布" class="submit" /></td>
</tr>
</table>
</div>
