<?php if(!defined('IN_UCHOME')) exit('Access Denied');?><?php subtplcheck('template/default/space_introduce_view|template/default/header|template/default/space_menu|template/default/space_click|template/default/space_comment_li|template/default/footer', '1370333880', 'template/default/space_introduce_view');?><?php $_TPL['titles'] = array($introduce['subject'], '企业介绍'); ?>
...
<?php $_TPL['spacetitle'] = "企业介绍";
	...
	$_TPL['spacemenus'][] = "<a href=\"space.php?uid=$space[uid]&do=introduce&id=$introduce[introduceid]\">查看企业介绍</a>"; ?>
...
<div class="title">
<h1<?php if($introduce['magiccolor']) { ?> class="magiccolor<?=$introduce['magiccolor']?>"<?php } ?>><?=$introduce['subject']?></h1>
<a href="cp.php?ac=introduce" class="r_option" style="padding: 0 5px; height: 24px; border: none; background: #FF8901; color: #FFF; line-height: 20px; letter-spacing: 1px; cursor: pointer; ">发布</a>
<?php if($introduce['friend']) { ?>
<span class="r_option locked gray"><?=$friendsname[$introduce['friend']]?></span>
<?php } ?>
<?php if($introduce['hot']) { ?><span class="hot"><em>热</em><?=$introduce['hot']?></span><?php } ?>
<?php if($introduce['friend']) { ?>
<span class="r_option locked gray">
<a href="space.php?uid=<?=$space['uid']?>&do=<?=$do?>&view=me&friend=<?=$introduce['friend']?>" class="gray"><?=$friendsname[$value['friend']]?></a>
</span>
<?php } ?>
<?php if($introduce['viewnum']) { ?><span class="gray">已有 <?=$introduce['viewnum']?> 次阅读</span><?php } ?>
&nbsp; <span class="gray"><?php echo sgmdate('Y-m-d H:i',$introduce[dateline],1); ?></span>
<?php if($introduce['tag']) { ?>
&nbsp; <a href="space.php?uid=<?=$introduce['uid']?>&do=tag">标签</a>:&nbsp;
<?php if(is_array($introduce['tag'])) { foreach($introduce['tag'] as $tagid => $tagname) { ?>
<a href="space.php?uid=<?=$introduce['uid']?>&do=tag&id=<?=$tagid?>"><?=$tagname?></a>&nbsp;
<?php } } ?>
<?php } ?>
</div>


<div id="introduce_article" class="article <?php if($introduce['magicpaper']) { ?> magicpaper<?=$introduce['magicpaper']?><?php } ?>">
<div class="resizeimg">
...
<a href="cp.php?ac=introduce&introduceid=<?=$introduce['introduceid']?>&op=edit">编辑</a><span class="pipe">|</span>
<a href="cp.php?ac=introduce&introduceid=<?=$introduce['introduceid']?>&op=delete" id="blog_delete_<?=$introduce['introduceid']?>" onclick="ajaxmenu(event, this.id)">删除</a>
<?php } ?>
...
<?php } ?>
评论 (<span id="comment_replynum"><?=$introduce['replynum']?></span> 个评论)</h2>
