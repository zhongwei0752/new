<?php if(!defined('IN_UCHOME')) exit('Access Denied');?><?php subtplcheck('template/default/space_job_list|template/default/header|template/default/footer', '1370333929', 'template/default/space_job_list');?><?php $_TPL['titles'] = array('人才招聘'); ?>
...
<?php } else { ?>
<div class="c_form">还没有相关的人才招聘。</div>
<?php } ?>
</div>
<?php } else { ?>
...
<ul class="tabs">
<li<?=$actives['me']?>><a href="space.php?uid=<?=$space['uid']?>&do=<?=$do?>&view=me"><span>人才招聘</span></a></li>
<li class="null"  style="margin-left:670px;"><a href="cp.php?ac=job">发布</a></li>
</ul>
</div>		
...
<h4><a href="space.php?uid=<?=$value['uid']?>&do=<?=$do?>&id=<?=$value['jobid']?>" <?php if($value['magiccolor']) { ?> class="magiccolor<?=$value['magiccolor']?>"<?php } ?>><?=$value['subject']?></a></h4>
</div>	


<div class="detail image_right l_text" id="blog_article_<?=$value['jobid']?>">
<?php if($value['pic']) { ?><p class="image"><a href="space.php?uid=<?=$value['uid']?>&do=job&id=<?=$value['jobid']?>"><img src="<?=$value['pic']?>" alt="<?=$value['subject']?>" /></a></p><?php } ?>
<?=$value['message']?>
</div>
<div class="gray">
...
<?php if($value['viewnum']) { ?><a href="space.php?uid=<?=$value['uid']?>&do=<?=$do?>&id=<?=$value['jobid']?>">阅读: <?=$value['viewnum']?>次</a><span class="pipe">|</span><?php } else { ?>阅读: 0次<span class="pipe">|</span><?php } ?>
<?php if($value['replynum']) { ?><a href="space.php?uid=<?=$value['uid']?>&do=<?=$do?>&id=<?=$value['jobid']?>#comment">评论: <?=$value['replynum']?> 个</a><?php } else { ?>评论: 0次<?php } ?>
</div>
</div>
</li>
<br/>
